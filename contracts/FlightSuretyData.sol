...
contract FlightSuretyData {
    ...
    // TODO: finish
    event OperationalStatus(bool status);
     ...
     *      Can only be called from FlightSuretyApp contract, can be used to
             override consensus voting
     *
     */

    function registerAirline(
        address airlineAddress ) external requireIsOperational requireCallerAuthorized {
        require(
    ...
    function vote(address sender, address airlineAddress)
        ...
        requireIsOperational
        requireCallerAuthorized
        returns (bool succeeded, uint256 votes)
    {
        require(
            airlines[airlineAddress].registered == false,
            "Airline is already registered"
